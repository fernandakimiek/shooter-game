const spaceShip = document.querySelector('.player-shooter')
...
function moveUp() {
  let topPosition = getComputedStyle(spaceShip).getPropertyValue('top')
  if (topPosition === '0px') {
    return
  } else {
...
function moveLaser(laser) {
  let laserInterval = setInterval(() => {
    ...
    aliens.forEach((alien) => {
      if (checkLaserCollison(laser, alien)) {
...
function checkLaserCollison(laser, alien) {
